...
const CONTRACT_ADDRESS = '0x36Ec7Dfeb95dB7680Ac4657E72428F56a826F51A'

const App = () => {
  // ユーザーのウォレットアドレスを格納するために使用する状態変数を定義します。
  const [currentAccount, setCurrentAccount] = useState('')
  const [currentTokenIds, setCurrentTokenIds] = useState(0)
  ...
  //   }
  // }
  const getCurrentTotalIds = async () => {
    try {
      const { ethereum } = window
      if (ethereum) {
        const provider = new ethers.providers.Web3Provider(ethereum)
        ...
        const id = await connectedContract.getCurrentTokenIds()
        if (!id) {
          return
        }
        setCurrentTokenIds(id.toNumber() - 1)
      } else {
        console.log("Ethereum object doesn't exist!")
      }
    } catch (error) {
  ...
  const renderMintUI = () => (
    <>
      ...
      <div className='mint-count'>
        <div className='counter'>{currentTokenIds}</div>
        <div className='slash'>/</div>
        <div>{TOTAL_MINT_COUNT}</div>
      </div>
      <div className='remaining'>LAST:{TOTAL_MINT_COUNT - currentTokenIds}</div>
      <div className='your-collection'>あなたのコレクションを見てみよう🔥</div>
      <button className='cta-button opensea-button'>
        <a href={`https://rinkeby.rarible.com/user/${currentAccount}`}>
          コレクションを表示
        </a>
      </button>
    </>
      ...
      <div className='App'>
        ...
        <div className='container'>
          <div className='header-container'>
            ...
            <p className='sub-text'>🎥 OK❕ あなたの次回、監督映画作品のタイトルを決めよう🎥</p>
            {/*条件付きレンダリング。
          // すでにウォレット接続されている場合は、
          // Mint NFT を表示する。*/}
            {currentAccount === ''
